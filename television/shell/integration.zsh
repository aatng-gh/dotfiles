...
'--preview-border=[Sets the preview panel border type.]:PREVIEW_BORDER:(none plain rounded thick)' \
'(--no-preview)--preview-padding=[Sets the preview panel padding.]:STRING:_default' \
...
'--input-position=[Input bar position.]:INPUT_POSITION:(top bottom)' \
'--input-border=[Sets the input panel border type.]:INPUT_BORDER:(none plain rounded thick)' \
'--input-padding=[Sets the input panel padding.]:STRING:_default' \
'--results-border=[Sets the results panel border type.]:RESULTS_BORDER:(none plain rounded thick)' \
'--results-padding=[Sets the results panel padding.]:STRING:_default' \
...
'-t+[The application'\''s tick rate.]:INT:_default' \
'--tick-rate=[The application'\''s tick rate.]:INT:_default' \
...
'--expect=[Keys that can be used to confirm the current selection in addition to the default ones (typically \`enter\`).]:STRING:_default' \
...
'--no-sort[Disable automatic sorting of entries based on match quality.]' \
'(--no-preview)--cache-preview[Whether to cache the preview command output for each entry.]' \
...
'(--no-preview)--preview-word-wrap[Enables preview panel word wrap.]' \
'(--no-preview)--hide-preview-scrollbar[Hide preview panel scrollbar.]' \
'(--hide-status-bar --show-status-bar)--no-status-bar[Disable the status bar entirely on startup.]' \
...
'-h[Print help]' \
'--help[Print help]' \
':shell:(bash zsh fish power-shell cmd nu)' \
&& ret=0
;;
(update-channels)
_arguments "${_arguments_options[@]}" : \
'--force[Force update on unsupported and already existing channels]' \
'-h[Print help]' \
'--help[Print help]' \
&& ret=0
;;
...
(( $+functions[_tv__completions_commands] )) ||
_tv__completions_commands() {
    local commands; commands=()
    _describe -t commands 'tv completions commands' commands "$@"
}
...
(( $+functions[_tv__help__completions_commands] )) ||
_tv__help__completions_commands() {
    local commands; commands=()
    _describe -t commands 'tv help completions commands' commands "$@"
}
